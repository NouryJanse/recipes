...
import { createRecipe, deleteRecipe, getRecipe, getRecipes, updateRecipe } from "../../models/recipes";
...
router.get("/api/recipes/:id", async (req, res) => {
  try {
    const recipe: Recipe = await getRecipe(Number.parseInt(req.params.id));
...
router.put("/api/recipes/:id", async (req, res) => {
  try {
    ...
    //   return reply.code(HTTP_CODES.UNPROCESSABLE_ENTITY).send({ message: error.message });
    // }
    return res.status(HTTP_CODES.INTERNAL_SERVER_ERROR).send({ message: ERROR_MESSAGES.INTERNAL_SERVER_ERROR });
  }
});
